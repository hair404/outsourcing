...
public class ProjectDao {
	...
	public void insertPrj(String name, Integer tag, Integer sub_tag, String img, Date releaseTime, String info,
			Integer state, Integer ifAd, Date deadline, float price, Integer companyId, String uuid, String entity) {
		String sqlCom = "insert into project(prjname,tag,sub_tag,img,releaseTime,info,state,ifAd,deadline,price,companyId,id,entity) "
				+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
		jdbcTemplate.update(sqlCom, name, tag, sub_tag, img, releaseTime, info, state, ifAd, deadline, price,
				companyId,uuid,entity);
	}
